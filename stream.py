...
# 3️⃣ محرك FFmpeg (الحل الجذري والذكي للمزامنة)
ffmpeg_cmd = [
    'ffmpeg', '-y',
    
    # 🟢 السر الأول: استخدام توقيت ساعة السيرفر الحقيقية لختم اللحظة لمنع أي طرف من الجري أسرع من الآخر
    '-use_wallclock_as_timestamps', '1',
    '-thread_queue_size', '8192',
    '-f', 'x11grab', '-draw_mouse', '0', '-framerate', '30', '-video_size', '720x1400', '-i', os.environ['DISPLAY'],
    
    '-use_wallclock_as_timestamps', '1',
    '-thread_queue_size', '8192',
    '-f', 'pulse', '-i', 'default',
    
    # 🟢 السر الثاني: دمج (قص الصورة + توحيد نقطة البداية STARTPTS + مزامنة الصوت) في مسار واحد معقد لا ينفصل
    '-filter_complex', '[0:v]crop=720:1280:0:120,setpts=PTS-STARTPTS[v];[1:a]asetpts=PTS-STARTPTS,aresample=async=1:min_hard_comp=0.100000[a]',
    '-map', '[v]', '-map', '[a]',
    
    # 🟢 السر الثالث: 30 فريم بدلاً من 60 + تقليل البتريت لـ 3000 لمنع السيرفر من إسقاط الإطارات
